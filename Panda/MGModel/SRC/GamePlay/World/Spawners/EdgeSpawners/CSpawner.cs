...
// The spawner for the CClass mob
public class CSpawner : ISpawner
{
    ...
    public CSpawner(GameServiceContainer SERVICES)
    {
        Services = SERVICES;
    ...
    public void SpawnMob(Vector2 OFFSET, Grid grid)
    {
        ...
        if (gridLoc is { IsFilled: false }) World.AddMob?.Invoke(new CClass(spawnPosition, Services));
    }
}