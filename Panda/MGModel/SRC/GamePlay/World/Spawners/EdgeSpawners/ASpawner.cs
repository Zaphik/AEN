...
// The spawner for the AClass mob
public class ASpawner : ISpawner
{
    ...
    public ASpawner(GameServiceContainer SERVICES)
    {
        Services = SERVICES;
    ...
    public void SpawnMob(Vector2 OFFSET, Grid grid)
    {
        ...
        if (gridLoc is { IsFilled: false }) World.AddMob?.Invoke(new AClass(spawnPosition, Services));
    }
}